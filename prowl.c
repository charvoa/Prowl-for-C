...
static int prowl_get_remaining_code(char* response);
...
int prowl_push_msg(char* api_key, int priority, char* application_name, char* event_name, char* description)
{
  ...
  printf("Remaining calls : %d\n", prowl_get_remaining_code(response));
...
static int prowl_get_remaining_code(char* response)
{
  ...
  start = strstr(start, "remaining");
  
  /* invalid response */
  if (start == NULL) return -1;
  
  /* failed to parse code */
  if (sscanf(start, "remaining=\"%d\"", &code) != 1) return -1;
